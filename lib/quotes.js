...
async function fetchCryptoQuotes() {
  const symbols = HOT_ASSETS.crypto.map(a => a.symbol);
  const raw = await binance.getHotSpotQuotes(symbols, 40);
  const result = {};
  for (const a of HOT_ASSETS.crypto) {
    const parsed = binance.parseBinanceSymbol(a.symbol);
    if (!parsed) continue;
    const key = parsed.product === 'spot' ? parsed.symbol : parsed.displaySymbol;
    const fromHot = raw[parsed.symbol];
    const live = fromHot || await binance.getQuote(a.symbol);
    if (live?.price > 0) {
      result[a.symbol] = { ...live, symbol: a.symbol, name: a.name || live.name };
    }
  }
  return result;
}
...
async function fetchAllQuotes() {
  ...
  for (const u of HOT_ASSETS.usstocks) {
    ...
    if (yq?.price > 0) {
      sinaRaw[u.sinaCode] = { ...yq, sinaCode: u.sinaCode, symbol: u.symbol };
    }
  }
