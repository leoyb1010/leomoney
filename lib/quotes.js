...
setInterval(() => {
  for (const source of Object.keys(apiHealth)) {
    if (!apiHealth[source].ok && apiHealth[source].lastFail && Date.now() - apiHealth[source].lastFail > 5 * 60 * 1000) {
      apiHealth[source].ok = true;
      apiHealth[source].failCount = 0;
...
module.exports = {
  ...
  searchSymbols, fetchAnyQuote, normalizeCode, getApiHealth,
};
