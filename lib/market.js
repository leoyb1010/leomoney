...
function getMarketStatusUS() {
  ...
  return { market: '权益', isOpen, status, nextEvent };
}

function getMarketStatusCrypto() {
  // 加密市场 7×24
  return { market: '数字资产', isOpen: true, status: '交易中', nextEvent: '持续交易' };
}
...
function getMarketStatus() {
  ...
  const primary = a.isOpen ? 'A股交易中' : (hk.isOpen ? '港股交易中' : (us.isOpen ? '权益交易中' : (crypto.isOpen ? '数字资产交易中' : '已收盘')));
