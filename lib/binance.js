...
async function httpJson(url, timeoutMs = 12000) {
  if (process.env.https_proxy || process.env.HTTPS_PROXY || process.env.http_proxy || process.env.HTTP_PROXY) {
    return curlJson(url, timeoutMs);
  }
  return new Promise((resolve, reject) => {
    const req = https.get(url, {
      family: 4,
      headers: { 'User-Agent': 'LeoMoney/4.0 BinanceQuotes', Accept: 'application/json' },
      timeout: timeoutMs,
    }, (res) => {
      let data = '';
...
function curlJson(url, timeoutMs = 12000) {
  return new Promise((resolve, reject) => {
    execFile('curl', ['-fsSL', '--max-time', String(Math.ceil(timeoutMs / 1000)), url], {
...
async function getKlines(symbol, scale = 5, limit = 80) {
  ...
  return rows.map(row => {
    ...
    return {
      ...
      label: ['1d', '1w', '1M'].includes(interval)
