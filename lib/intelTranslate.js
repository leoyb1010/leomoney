...
function cacheFile() {
  return path.join(getDataDir(), 'intel-cache.json');
}
...
function loadDiskCacheOnce() {
  ...
  try {
    ...
    Object.entries(data.translations || {}).slice(-CACHE_MAX).forEach(([key, value]) => {
      if (typeof key === 'string' && typeof value === 'string') titleCache.set(key, value);
    });
  } catch {
    // first run
  }
...
function scheduleCacheFlush() {
  ...
  flushTimer = setTimeout(() => {
    ...
    try {
      ...
      const translations = Object.fromEntries(titleCache.entries());
      fs.writeFileSync(cacheFile(), JSON.stringify({ translations, updatedAt: new Date().toISOString() }, null, 2));
    } catch (err) {
      console.warn('[IntelTranslate] cache flush failed:', err.message);
    }
  }, 500).unref?.();
}
...
async function translateTitlesWithLLM(entries) {
  ...
  const payload = entries.map((e, i) => ({
    ...
    snippet: e.snippetEn || e.snippet || '',
  }));
  const systemPrompt = `你是财经新闻标题翻译器。将用户给出的英文新闻标题翻译成简洁、准确的中文标题。
要求：
1. 标题和摘要用自然中文表达。
2. 不要强制翻译金融产品、股票、ETF、指数、币种、交易对、合约或公司名称；Apple、NVIDIA、BTC、ETH、SPY、QQQ、S&P 500 这类名称可以直接保留英文。
3. 不添加评论，不改变原意。
4. 标题不超过 80 字，摘要不超过 160 字。
只输出 JSON：{"items":[{"i":0,"titleZh":"中文标题","snippetZh":"中文摘要"}]}，不要 markdown。`;
  ...
  try {
    ...
    for (const row of list) {
      ...
      const titleZh = String(row.titleZh || row.zh || row.title || '').trim();
      const snippetZh = String(row.snippetZh || row.summaryZh || row.snippet || '').trim();
      if (!Number.isNaN(idx) && titleZh) map.set(idx, { titleZh, snippetZh });
    }
    return map;
  } catch (err) {
...
async function localizeIntelItems(items = []) {
  ...
  const normalized = items.map(item => {
    ...
    return {
      ...
      snippet: item.snippet && isMostlyEnglish(item.snippet)
        ...
        : item.snippet,
      snippetZh: item.snippet && isMostlyEnglish(item.snippet)
        ? glossaryTranslate(item.snippet)
        : item.snippet,
      snippetEn: item.snippet && isMostlyEnglish(item.snippet) ? item.snippet : null,
      translationQuality: item.titleEn ? 'glossary' : 'native',
    };
  });

  const needLlm = normalized
    .map((item, idx) => ({ idx, item }))
    .filter(({ item }) => item.titleEn && (item.title === item.titleEn || item.snippetEn))
    .slice(0, 14);
  ...
  for (const { idx, item } of needLlm) {
    ...
    const translated = llmMap.get(pos);
    if (!translated?.titleZh) continue;
    cacheSet(item.titleEn, translated.titleZh);
    normalized[idx] = {
      ...normalized[idx],
      title: translated.titleZh,
      titleZh: translated.titleZh,
      snippet: translated.snippetZh || normalized[idx].snippet,
      snippetZh: translated.snippetZh || normalized[idx].snippetZh,
      translationQuality: 'llm',
    };
  }
  return normalized;
}
