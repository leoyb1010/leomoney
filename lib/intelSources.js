...
function parseRss(xml, limit = 10, source = 'RSS') {
...
function buildNewsQuery({ query, symbol, category }) {
  ...
  if (/^[A-Z]{1,5}$/.test(sym) && !['BTC', 'ETH', 'SOL', 'BNB'].includes(sym)) {
    return { searchQ: `${sym} stock market`, zhQ: `${sym} 权益 公司` };
  }
  if (/^\d{5,6}$/.test(sym)) {
...
async function fetchGoogleNews(query, { lang = 'en', limit = 8 } = {}) {
  ...
  try {
    const xml = await httpGet(url, 6000);
    return parseRss(xml, limit, 'Google News');
  } catch (err) {
...
async function fetchRssFeed(url, source, { limit = 8 } = {}) {
  try {
    const xml = await httpGet(url, 6000);
    return parseRss(xml, limit, source);
  } catch (err) {
    console.warn(`[IntelSources] ${source} failed:`, err.message);
    return [];
  }
}

async function fetchCryptoFeeds() {
  const [coinDesk, coinTelegraph] = await Promise.all([
    fetchRssFeed('https://www.coindesk.com/arc/outboundfeeds/rss/', 'CoinDesk', { limit: 6 }),
    fetchRssFeed('https://cointelegraph.com/rss', 'Cointelegraph', { limit: 6 }),
  ]);
  return dedupeHeadlines([...coinDesk, ...coinTelegraph]).slice(0, 8);
}
...
async function fetchEastmoneyNews(keyword) {
  try {
    ...
    const data = await httpGet(url, 6000);
...
async function fetchHeadlines({ query, symbol, category }) {
  ...
  const isCrypto = category === 'crypto' || /USDT|BTC|ETH|SOL|BNB/i.test(String(symbol || query || ''));
  const [enNews, zhNews, emNews, cryptoNews, tavily] = await Promise.all([
    fetchGoogleNews(searchQ, { lang: 'en', limit: 8 }),
    fetchGoogleNews(zhQ, { lang: 'zh', limit: 6 }),
    fetchEastmoneyNews(zhQ),
    isCrypto ? fetchCryptoFeeds() : Promise.resolve([]),
  ...
  // 中文源优先，减少英文标题占主导
  const news = dedupeHeadlines([...zhNews, ...emNews, ...enNews, ...cryptoNews]);
  const meta = {
    googleEn: enNews.length,
    googleZh: zhNews.length,
    eastmoney: emNews.length,
    cryptoRss: cryptoNews.length,
...
function ruleBasedSummary({ query, quote, news, search }) {
  ...
  const top = headlines.slice(0, 3).map(h => `• ${h.titleZh || h.title}`).join('\n');
  const summary = [
