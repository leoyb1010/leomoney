...
const { runAutomation } = require('../../src/server/automation/automationEngine');
...
async function analyze(symbol, strategyId, observation, intel) {
  ...
  // 广播 SSE
  sseService.broadcast('agent', 'analysis', { symbol, action: parsed.action.action, confidence: parsed.action.confidence });
...
async function execute(proposalId, forceLevel3 = false) {
  ...
  const automationMode = runtime.agentPaperExecutionEnabled ? 'paper_execution' : 'dry_run';
  const automationResult = await runAutomation({
    id: proposal.decisionId || proposal.id,
    type: 'agent',
    accountId: account?.accountId || 'default',
    symbol: proposal.symbol,
    payload: {
      decision: {
        action: proposal.action === '买入' ? 'BUY' : proposal.action === '卖出' ? 'SELL' : 'HOLD',
        symbol: proposal.symbol,
        confidence: Number(proposal.confidence || 0),
        thesis: proposal.reason || 'Agent proposal routed through execution gate',
        evidenceRefs: [proposal.signalId, proposal.decisionId].filter(Boolean),
        positionSize: { mode: 'shares', value: Number(proposal.qty || 0) },
        riskFlags: proposal.riskLevel === '高' ? ['HIGH_RISK'] : [],
      },
    },
  }, { mode: automationMode });

  const result = {
    success: !!automationResult.gate?.executed,
    dryRun: automationMode !== 'paper_execution',
    mode: automationMode,
    runId: automationResult.runId,
    gate: automationResult.gate,
    error: automationResult.error || automationResult.gate?.reason || automationResult.gate?.riskCheck?.reasons?.join('; '),
  };

  proposal.executionResult = result;
  proposal.status = result.success ? 'executed' : (automationMode === 'dry_run' ? 'approved' : 'failed');
  proposal.executedAt = new Date().toISOString();

  // 记录到熔断器
  let pnl = 0, pnlPct = 0;
  if (result.success && result.gate?.result?.realizedPnl !== undefined) {
    pnl = Number(result.gate.result.realizedPnl) || 0;
  ...
  // 止损止盈
  if (result.success && runtime.agentPaperExecutionEnabled && (forceLevel3 || breaker.currentLevel === 3)) {
  ...
  if (audits.length > 0) {
    ...
    audits[0].submittedOrderIds = result.gate?.result?.orderId ? [result.gate.result.orderId] : [];
  }
