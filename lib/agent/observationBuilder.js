...
// 注意：不在此处 require('../scheduler')，避免循环依赖 signalEngine→observationBuilder→scheduler
// getAgentConfig 改为延迟加载
const { D, toMoney } = require('../../src/server/domain/money');

function _getAgentConfig() {
  return require('../scheduler').getAgentConfig();
}
