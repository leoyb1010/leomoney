...
// 不支持或不稳定支持 response_format / temperature 的模型
const REASONER_MODELS = ['deepseek-reasoner', 'deepseek-r1'];
const NO_RESPONSE_FORMAT_MODELS = ['deepseek-v4'];
...
function supportsResponseFormat(model, providerName) {
  if (!model) return true;
  const lower = model.toLowerCase();
  if (providerName === 'deepseek' && NO_RESPONSE_FORMAT_MODELS.some(r => lower.includes(r))) return false;
...
async function askLLM(systemPrompt, userMessage, options = {}) {
  ...
  const maxRetries = options.maxRetries ?? runtimeConfig.llm.maxRetries ?? MAX_RETRIES;
  const providerName = process.env.LLM_PROVIDER || 'deepseek';
  const apiKey = process.env.LLM_API_KEY;
  const config = _resolveProviderConfig(providerName);
  ...
  const model = _selectModel(config, options);
  const reasoner = isReasonerModel(model);
  const responseFormat = supportsResponseFormat(model, providerName);
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      ...
      const rawResponse = await _makeRequest(config.endpoint, apiKey || 'ollama', body, providerName, runtimeConfig.llm.timeoutMs);
...
function _normalizeChatEndpoint(baseUrl) {
...
function _resolveProviderConfig(providerName) {
  const config = PROVIDERS[providerName];
  if (!config) return null;
  const baseUrl = process.env.LLM_BASE_URL || process.env.OPENAI_BASE_URL;
  if (!baseUrl) return config;
  return { ...config, endpoint: _normalizeChatEndpoint(baseUrl) };
}

function _selectModel(config, options = {}) {
  if (options.model) return options.model;
  if (options.modelRole === 'deep') {
    return process.env.RESEARCH_DEEP_MODEL || process.env.LLM_DEEP_MODEL || process.env.LLM_MODEL || config.model;
  }
  if (options.modelRole === 'quick') {
    return process.env.RESEARCH_QUICK_MODEL || process.env.LLM_QUICK_MODEL || process.env.LLM_MODEL || config.model;
  }
 ...
 * 底层 HTTP 请求
 */
function _makeRequest(endpoint, apiKey, body, providerName, timeoutMs = 30000) {
  ...
  return new Promise((resolve, reject) => {
    ...
    req.on('error', reject);
    req.setTimeout(timeoutMs, () => { req.destroy(); reject(new Error(`LLM 请求超时(${Math.round(timeoutMs / 1000)}s)`)); });
    req.write(body);
    req.end();
  });
}
...
function getLLMInfo() {
  ...
  const quickModel = process.env.RESEARCH_QUICK_MODEL || process.env.LLM_QUICK_MODEL || model;
  const deepModel = process.env.RESEARCH_DEEP_MODEL || process.env.LLM_DEEP_MODEL || model;
  const runtimeConfig = getRuntimeConfig();
  return {
    ...
    quickModel,
    deepModel,
    baseUrl: runtimeConfig.llm.baseUrl || config?.endpoint || null,
    anthropicBaseUrl: runtimeConfig.llm.anthropicBaseUrl || null,
    reasoner: isReasonerModel(model),
    quickReasoner: isReasonerModel(quickModel),
    deepReasoner: isReasonerModel(deepModel),
    ready: isLLMReady(),
    timeoutMs: runtimeConfig.llm.timeoutMs,
    maxRetries: runtimeConfig.llm.maxRetries,
    mode: isLLMReady() ? 'live' : 'safe_hold_no_key',
    estimatedCost: {
      currency: 'provider-dependent',
      note: 'LeoMoney does not know provider billing in real time; monitor provider dashboards and set low retry/timeouts for beta.',
    },
  };
}
