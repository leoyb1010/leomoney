 ...
 * v1.9.0: 不再直接执行交易，改为生成信号+方案，由 signalEngine 流程统一管控
 */
async function runStrategyScan() {
  ...
  for (const symbol of symbols) {
    try {
      ...
      // 只返回决策建议，不直接执行（安全：避免绕过风控/熔断）
      results.push({ symbol, name: quote.name, decision: parsed.action, parsed });
    } catch (err) {
...
async function analyzeSingle(symbol) {
  ...
  // analyzeSingle 只做分析，不执行交易 — 返回信号供前端展示
  return {
    success: true,
    symbol,
    name: quote.name,
    ...
    executed: false,
    reason: 'analyze 为只读接口，不执行交易。如需执行请使用信号/方案流程。',
    parsed,
    observation,
