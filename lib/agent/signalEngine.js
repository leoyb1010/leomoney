 ...
 *   6. v1.9.0: 多账户隔离 — Map<accountId, {signals, proposals}>
 */
...
// ── 多账户隔离：Map<accountId, {signals, proposals}> ──
...
function _getStore(accountId) {
  ...
  if (!_stores.has(id)) {
    _stores.set(id, { signals: [], proposals: [] });
  }
  return _stores.get(id);
}

/**
 * 清除指定账户的信号/方案数据（账户删除时调用）
 */
function removeStoreForAccount(accountId) {
...
async function executeProposal(proposalId, forceLevel3 = false) {
  ...
  if (result.success && result.realizedPnl !== undefined) {
    ...
    if (tradeAmount.gt(0)) {
      pnlPct = D(pnl).div(tradeAmount).toNumber();
    }
  }
  breaker.recordTrade({
  ...
  // 风控引擎记录交易（成功+失败都计数）
  if (!result.success) {
    riskManager.recordTrade(false);
  }

  // 执行成功 + Level3 时生成止损止盈单
  if (result.success) {
    riskManager.recordTrade(true);
