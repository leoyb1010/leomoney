...
const { gatherIntelligence, searchWeb } = require('./agent/eyes');
...
function loadWatch() {
  try {
    ...
    watchTopics = watchTopics.length ? watchTopics : [
      ...
      { id: 'w_us', query: '美股 美联储 利率', symbol: 'AAPL', enabled: true },
    ];
  }
}
...
async function scanIntel({ query, symbol }) {
  ...
  const [intel, quote] = await Promise.all([
    gatherIntelligence(primary || q, q),
    primary ? fetchQuoteSnapshot(primary) : null,
  ]);

  let extraSearch = [];
  if (!intel.search?.length) {
    extraSearch = await searchWeb(`${q} 最新消息`).catch(() => []);
  }
  ...
  const entry = pushFeed({
    ...
    quote,
    news: intel.news || [],
    search: [...(intel.search || []), ...extraSearch].slice(0, 8),
    agentLinked: false,
  });
...
async function analyzeIntel({ query, symbol }) {
  ...
  if (!llmReady) {
    entry.analysis = {
      summary: 'LLM 未配置（设置 LLM_API_KEY）。已返回原始新闻与搜索结果，可在 Agent 页配置后使用「Agent 解读」。',
      action: 'HOLD',
      confidence: 0,
      llmReady: false,
    };
  ...
  const systemPrompt = `你是 LeoMoney 交易情报分析师。根据新闻与搜索摘要，用中文输出 JSON：
